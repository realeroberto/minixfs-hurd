/* Pager for the MINIX file system

   Copyright (C) 1994,95,96,97,98,99,2000,02 Free Software Foundation, Inc.

   Converted for ext2fs by Miles Bader <miles@gnu.org>

   Slightly modified for the MINIX file system from the original for the
   ext2 file system by Roberto Reale <rober.reale@gmail.com>.
...
static struct minixfs_pager_stats pager_stats;

#define STAT_INC(field)							      \
do { spin_lock (&pager_stats.lock);				      \
     pager_stats.field++;	       				      \
     spin_unlock (&pager_stats.lock); } while (0)
	    ...
	    block_t *block, struct rwlock **lock)
{
  ...
  err = minixfs_getblk (node, offset >> log2_block_size, 0, block);
  if (err == EINVAL)
		      ...
		      void **buf, int *writelock)
{
  ...
  minixfs_debug ("reading one page for the pager backing node %Ld at "
		 "offset %lu", node->cache_id, page);
...
/* Write any pending blocks in PB.  */
static error_t
  ...
  if (pb->num > 0)
    {
      ...
      minixfs_debug ("writing block %u[%ld]", pb->block, pb->num);
...
file_pager_write_page (struct node *node, vm_offset_t offset, void *buf)
{
  ...
  minixfs_debug ("writing inode %d page %d[%d]", node->cache_id, offset, left);
...
disk_pager_write_page (vm_offset_t page, void *buf)
{
  ...
  minixfs_debug ("writing disk page %d[%d]", page, length);
  ...
  if (pager->type == DISK)
      ...
      if (!err)
	{
	  ...
	  while (left > 0)
	    {
	      block_t disk_block;
	      err = minixfs_getblk (node, block++, 1, &disk_block);
	      if (err)
		break;
	      left -= block_size;
	    }
	...
	minixfs_debug ("made page %u[%lu] in inode %d partially writable",
		       page, node->allocsize - page, node->cache_id);
      else
	minixfs_debug ("made page %u[%u] in inode %d writable",
      ...
      if (err == ENOSPC)
	minixfs_warning ("This filesystem is out of space, and will now crash.  Bye!");
      else if (err)
	minixfs_warning ("inode=%Ld, page=0x%zx: %s",
...
diskfs_grow (struct node *node, off_t size, struct protid *cred)
{
  ...
  if (size > node->allocsize)
      ...
      if (new_end_block > end_block)
	{
	  ...
	  minixfs_debug ("growing inode %d to %lu bytes (from %lu)",
	  ...
	  if (dn->last_page_partially_writable
		...
		(old_page_end_block > new_end_block
		 ...
		 : old_page_end_block);
	      ...
	      minixfs_debug ("extending writable page %u by %d blocks"
			     "; first new block = %u",
			     trunc_page (old_size),
		...
		{
		  block_t disk_block;
		  err = minixfs_getblk (node, end_block++, 1, &disk_block);
		}
	      diskfs_end_catch_exception ();
      ...
      minixfs_debug ("new size: %ld%s.", new_size,
		     (dn->last_page_partially_writable
		      ? " (last page writable)": ""));
      if (err)
	minixfs_warning ("inode=%Ld, target=%Ld: %s",
...
create_disk_pager (void)
{
  ...
  if (!upi)
    minixfs_panic ("can't create disk pager: %s", strerror (errno));
  upi->type = DISK;
